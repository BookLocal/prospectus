...
\begin{itemize}
 ...
 \item payment processor and point of sale;
 \item report generator; and
...
\begin{flushleft}
The layout of this paper provides the motivation for BookLocal in section 2, a description of the current ecosystem in section 3, a detailed account of the features we will provide in section 4, and the steps toward full implementation in section 5.
\end{flushleft}
...
\begin{itemize}
 \item \underline{\textit{Value:}} A good property management system can make the life of a hotelier much easier by automating and centralizing many of the daily operations. 
  \item \underline{\textit{Cost:}} A lack of customization and complicated interfaces makes it difficult to train new workers. 
  \end{itemize}

\newpage

\subsection{Proposed Ecosystem}
Since BookLocal will act as a property management system, channel manager, and payment processor for the hotel and provide metasearch capabilities for the traveler, much of the current ecosystem can be bypassed. As such, figure \ref{bl} represents our vision for the first stage of implementation. 
...
\subsection{Traveler}
With BookLocal the traveler will be able to search, compare, and book a hotel room. Additionally, we will incorporate a wallet feature that lets the guest use their phone to pay for dinner, events, or other travel related activities.  
...
\begin{itemize}
 ...
 \item Send some amount less than the upfront asking rate (if booking well in advance) to hold your room for a period of time. This will likely require the remainder to be paid closer to the actual date and potentially this option will ultimately cost more. Basically any user paying in full upfront would ideally receive some discount compared to those who want to book in advance without paying in full. 
\end{itemize}
...
Guest management includes managing arrivals and departures with any special requests. Sub features involve messaging capability between the guest and hotel and different views to see arrivals and departures for different time scales (i.e. monthly view, weekly view, daily view, and so on).
...
Revenue management will act as a recommendation system to provide different pricing options during peak hours or otherwise. For the first time in history, the utilization of smart contracts can allow for conditional price setting (i.e. if time is after 10 pm then room rate equals \$80, else room rate equals \$100). 

\subsubsection{Payment Processor}
Upon checkout, BookLocal will finalize the guest's plasma chain, record the final balances, and transfer funds accordingly. This module will need to work seamlessly for someone paying with a credit card and incorporate a point of sale system with inventory management for hotel purchases (i.e. drinks, snacks, toiletry items, and so on).
...
In order to feasibly integrate into the current ecosystem, BookLocal will include channel management functionality to allow travelers the option of booking through an online travel agency while still creating a guest specific subchain on the BookLocal subchain.   
...
Once the BookLocal platform is successfully tested at the Exchange, we will open up the application for use at other independent hotels.
\subsection{Chain Hotels}
Finally, we will target chain hotels by adding permissioned information sharing features.
